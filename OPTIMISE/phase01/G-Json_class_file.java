...
class JSONWriter
{
    ...
    public void BuildJSON(String json)
    {
        try
        {
            FileWriter file = new FileWriter(JWpath + JWfile);
            file.write("{\n"+json+"\n"+"}");
            file.close();
...
class JSONReader
{
    ...
    public String JSON_String() throws IOException
    {
        String Path = JRpath + JRfile;
...
class JSONEditor
{
    ...
    private String JEpath;
    private String JEfile;

    public JSONEditor(String path, String file)
    {
        this.JEpath = path;
        this.JEfile = file;
    }
    ...
    public void deleteJSON_file()
    {
        String Path = JEpath + JEfile;
        File delfile = new File(Path);
        if(delfile.delete())
        {
            System.out.println("The JSON file at: " +Path+" has been deleted successfully");
        }
        else
        {
            System.out.println("The JSON file at: " +Path+" failed to be deleted");
        }
    }

    //Method used to delete an object within the .json file
    //This will also overwrite the file 
    public void deleteJSON_object(String JSONobj)throws IOException
    {
        String path = JEpath+JEfile;
        String getpath = JEpath+JEfile;
        
        String JsonSTR = new String(Files.readAllBytes(Paths.get(getpath)), StandardCharsets.UTF_8);
